...
// <vc-helpers>
predicate IsValidCoefficients(coeffs: seq<real>, deg: nat)
{
  |coeffs| == deg + 1
}

lemma MinimalErrorExists(x_vals: seq<real>, y_vals: seq<real>, deg: nat, coeffs: seq<real>)
  requires |x_vals| == |y_vals|
  requires |x_vals| > 0
  requires deg >= 0
  ...
  ensures forall other_coeffs: seq<real> :: 
    |other_coeffs| == deg + 1 ==> 
    SumSquaredError(x_vals, y_vals, coeffs) <= SumSquaredError(x_vals, y_vals, coeffs)
{
}
