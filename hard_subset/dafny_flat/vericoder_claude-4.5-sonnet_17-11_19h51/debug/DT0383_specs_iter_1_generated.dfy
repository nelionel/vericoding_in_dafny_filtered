...
// <vc-helpers>
predicate IsValidChebyshevCoeffs(coeffs: seq<real>, roots: seq<real>)
{
  |coeffs| == |roots| + 1 &&
  (|roots| > 0 ==> coeffs[|roots|] == Pow(2.0, 1 - |roots|)) &&
  (forall i :: 0 <= i < |roots| ==> EvalChebyshevPoly(coeffs, roots[i]) == 0.0)
}
...
function BuildPolyFromRoots(roots: seq<real>, idx: nat): seq<real>
  ...
  ensures idx > 0 ==> BuildPolyFromRoots(roots, idx)[idx] == Pow(2.0, 1 - idx)
  decreases idx
{
