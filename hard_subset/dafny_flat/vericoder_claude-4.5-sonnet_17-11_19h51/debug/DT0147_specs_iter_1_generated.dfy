...
function DFTElement(a: seq<Complex>, k: nat): Complex
  requires k < |a|
{
  ComplexSum(|a|, (j: nat) requires 0 <= j < |a| => 
    ComplexMul(a[j], ComplexExp(-2.0 * PI * (k as real) * (j as real) / (|a| as real))))
}

lemma DFTZeroCase(a: seq<Complex>)
  requires |a| > 0
  ensures DFTElement(a, 0) == ComplexSum(|a|, (j: nat) requires 0 <= j < |a| => a[j])
{
  reveal ComplexSum();
  assert forall j :: 0 <= j < |a| ==> ComplexExp(-2.0 * PI * 0.0 * (j as real) / (|a| as real)) == ComplexExp(0.0);
}
// </vc-helpers>
...
// <vc-code>
{
  ...
  forall k | 0 <= k < n
    ensures result[k] == ComplexSum(n, (j: nat) requires 0 <= j < n => 
      ComplexMul(a[j], ComplexExp(-2.0 * PI * (k as real) * (j as real) / (n as real))))
  {
