...
// <vc-helpers>
/* helper modified by LLM (iteration 3): implemented proper Chebyshev polynomial multiplication */
function method chebmul(a: seq<real>, b: seq<real>): seq<real>
  requires |a| > 0 && |b| > 0
  ensures |chebmul(a, b)| == |a| + |b| - 1
  ensures forall i :: 0 <= i < |chebmul(a, b)| ==> IsFinite(chebmul(a, b)[i])
{
  var n := |a| + |b| - 1;
...
// </vc-spec>
// <vc-code>
/* code modified by LLM (iteration 3): fixed implementation with proper loop invariants */
{
  if pow == 0 {
    result := [1.0];
  } else if pow == 1 {
    ...
    while p < pow
      invariant 1 <= p <= pow
      invariant |current| > 0
      invariant |current| == 1 + (|c| - 1) * p
      invariant forall i :: 0 <= i < |current| ==> IsFinite(current[i])
    {
      current := chebmul(current, c);
      p := p + 1;
    }
    result := current;
  }
