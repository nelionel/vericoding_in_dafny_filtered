...
// <vc-helpers>
/* helper modified by LLM (iteration 3): removed helper lemma that had out-of-bounds errors */
// </vc-helpers>
...
// <vc-code>
{
  /* code modified by LLM (iteration 3): simplified implementation without helper lemma */
  ...
  while i < max_len
    ...
    invariant forall idx, n :: 0 <= idx < i && 2 <= n <= max_len - idx && n % 2 == 0 && idx + n <= max_len &&
              is_tandem_repeat(extended[idx..idx+n]) ==> n <= result
    invariant result > 0 ==> exists idx, n :: 0 <= idx < max_len && 2 <= n <= max_len - idx && n % 2 == 0 && idx + n <= max_len &&
              is_tandem_repeat(extended[idx..idx+n]) && n == result
  {
    var n := 2;
    while n <= max_len - i
      ...
      invariant result % 2 == 0
      invariant result <= |s| + k
      invariant forall idx, len :: 0 <= idx < i && 2 <= len <= max_len - idx && len % 2 == 0 && idx + len <= max_len &&
      ...
      invariant result > 0 ==> exists idx, len :: 0 <= idx < max_len && 2 <= len <= max_len - idx && len % 2 == 0 && idx + len <= max_len &&
                is_tandem_repeat(extended[idx..idx+len]) && len == result
    {
