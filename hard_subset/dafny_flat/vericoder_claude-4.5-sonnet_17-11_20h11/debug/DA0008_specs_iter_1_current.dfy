...
// <vc-helpers>
function countMonotonic(n: int, m: int, target1: int, target2: int): ()
...
{
  if target1 <= 0 && target2 <= 0 then ()
  else if target1 <= 0 && target2 > 0 then ()
  else if target2 >= n * m then ()
  else if target1 >= n * m then ()
  else ()
}
