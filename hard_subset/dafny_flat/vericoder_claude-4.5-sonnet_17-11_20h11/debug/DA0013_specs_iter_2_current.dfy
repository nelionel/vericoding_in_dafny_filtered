...
// <vc-helpers>
/* helper modified by LLM (iteration 2): added lemmas to prove existence of min indices and minimum distance properties */
...
lemma FirstMinIsMin(arr: seq<int>)
    requires |arr| > 0
    ensures var first := FindFirstMinIndex(arr, 0); IsMinIndex(arr, first)
{
    MinIndexExists(arr);
}

lemma LastMinIsMin(arr: seq<int>)
    requires |arr| > 0
    ensures var last := FindLastMinIndex(arr, |arr| - 1); IsMinIndex(arr, last)
{
    MinIndexExists(arr);
}
...
lemma MinDistanceProperty(arr: seq<int>, firstMin: int, lastMin: int)
    requires |arr| > 0
    requires firstMin == FindFirstMinIndex(arr, 0)
    requires lastMin == FindLastMinIndex(arr, |arr| - 1)
